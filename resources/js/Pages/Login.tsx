      ...
      <div className="w-full max-w-[1280px] px-6 py-8 flex flex-col items-center justify-center z-10">
        ...
        <div className="w-full max-w-[440px] bg-white rounded-2xl p-8 sm:p-10 border border-gray-200 shadow-2xl shadow-gray-200/50">
          <div className="mb-8 text-center">
            ...
            <p className="text-gray-500 text-sm">Masukan kredensial Anda untuk masuk ke sistem manajemen.</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-5">
            
            <div className="flex flex-col gap-2">
              <label className="font-mono text-[10px] font-bold text-gray-500 uppercase tracking-wider pl-1">Username</label>
              <div className="relative group">
                <input 
                  ...
                  autoComplete="off"
                />
                <div className={`absolute right-4 top-1/2 -translate-y-1/2 ${errors.username ? 'text-[#bc000a]' : 'text-gray-400 group-focus-within:text-[#86DFCA]'}`}>
                  <span className="material-symbols-outlined" style={{ fontSize: '20px' }}>person</span> 
                </div>
              </div>
              {errors.username && (
                <span className="text-xs text-[#bc000a] pl-1 font-medium">{errors.username}</span>
              )}
            </div>

            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center px-1">
                <label className="font-mono text-[10px] font-bold text-gray-500 uppercase tracking-wider">Password</label>
              </div>
              <div className="relative group">
                <input 
                  name="password" 
              ...
              {errors.password && (
                <span className="text-xs text-[#bc000a] pl-1 font-medium">{errors.password}</span>
              )}
            </div>
            ...
            >
              {processing ? (
                ...
                <>
                  <span>MASUK</span>
                  <span className="material-symbols-outlined group-hover:translate-x-1 transition-transform" style={{ fontSize: '18px' }}>login</span>
                </>
              )}
            </button>
          </form>
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 text-center">
            <Link href="/" className="text-sm text-[#455d97] hover:text-[#bc000a] transition-colors font-medium flex items-center justify-center gap-2">
