        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden max-w-4xl">
          <form onSubmit={handleSubmit}>
            ...
            <div className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2">
                  ...
                  />
                  {errors.judul && <p className="text-[#F32923] text-xs mt-1">{errors.judul}</p>}
                </div>
                ...
                <div>
                  ...
                  />
                  {errors.author && <p className="text-[#F32923] text-xs mt-1">{errors.author}</p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  {errors.kategori && <p className="text-[#F32923] text-xs mt-1">{errors.kategori}</p>}
                </div>

                <div>
                  <label className="block text-sm font-bold text-[#203971] mb-2">Thumbnail / Gambar Sampul</label>
                  <input 
                    type="file" 
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Format: JPG, PNG, WEBP. Kosongkan jika tidak ingin mengubah gambar.</p>
                  
                  {berita.thumbnail && (
                    <div className="mt-3 flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg w-fit">
                      <img src={`/storage/${berita.thumbnail}`} alt="Thumbnail Saat Ini" className="w-16 h-16 object-cover rounded shadow-sm" />
                      <div className="text-xs text-gray-500">
                        <span className="font-bold text-gray-700 block">Gambar Saat Ini</span>
                        Akan terganti jika Anda mengupload file baru.
                      </div>
                    </div>
                  )}
                  {errors.thumbnail && <p className="text-[#F32923] text-xs mt-1">{errors.thumbnail}</p>}
                </div>
              </div>
              ...
              <div>
                ...
                />
                {errors.isi && <p className="text-[#F32923] text-xs mt-1">{errors.isi}</p>}
              </div>
            </div>
            ...
            <div className="px-8 py-5 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
              ...
              >
                <span className="material-symbols-outlined text-sm">arrow_back</span> Batal & Kembali
              </Link>
              <button 
                type="submit" 
                disabled={processing}
