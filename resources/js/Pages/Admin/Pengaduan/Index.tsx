...
export default function Index({
  ...
  const handleToggleForm = () => {
    if (
      confirm(
        `Apakah Anda yakin ingin ${isPengaduanOpen ? 'menutup' : 'membuka'} form pengaduan?`
      )
    ) {
      router.post(
        '/admin/pengaduan/toggle-status',
        {
            ...
            <tbody className="divide-y divide-gray-100">
              {pengaduans.data.length > 0 ? (
                pengaduans.data.map((pengaduan) => (
                  ...
                  >
                    <td className="p-4 text-xs font-bold font-mono text-[#203971]">
                    ...
                    <td className="p-4">
                      <div>
                        ...
                        <p className="text-xs font-mono text-gray-400">
                          {new Date(pengaduan.created_at).toLocaleDateString(
                            'id-ID',
                            {
                              year: 'numeric',
                              month: 'short',
                    ...
                    <td className="p-4">
                      <span
                        className={`px-2 py-0.5 text-[10px] font-bold rounded capitalize ${getKategoriStyle(pengaduan.kategori)}`}
                      >
                    ...
                    <td className="p-4">
                      <div>
                        <p className="text-xs font-semibold text-gray-700">
                          Angkatan {pengaduan.angkatan}
                        </p>
                        <p className="text-[11px] text-[#203971] font-mono truncate max-w-xs">
                          {pengaduan.mata_kuliah
                            ? pengaduan.mata_kuliah.nama_mk
                            : 'Umum / Tidak spesifik'}
                        </p>
                      </div>
                    </td>
                    <td className="p-4">
                      <span
                        className={`px-2.5 py-1 text-[10px] font-black font-mono tracking-wider rounded-full uppercase ${getStatusStyle(pengaduan.status)}`}
                      >
        ...
        {pengaduans.total > 0 && (
          <div className="flex justify-between items-center bg-white p-4 border border-gray-200 rounded-lg shadow-sm">
            <span className="text-xs text-gray-500 font-mono">
              Menampilkan {pengaduans.data.length} dari {pengaduans.total}{' '}
              pengaduan
            </span>
