...
export default function Show({ pengaduan, lampiranUrl }: PengaduanProps) {
  ...
  const handleTandaiDibahas = () => {
    router.post(
      `/admin/pengaduan/${pengaduan.id}/tandai-dibahas`,
      {},
      {
        preserveScroll: true,
      }
          ...
          <div className="space-y-6">
            <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
              ...
              <div className="text-center py-4 space-y-4">
                <span
                  className={`inline-block px-4 py-1.5 rounded-full text-xs font-black font-mono tracking-widest border uppercase ${getStatusStyle(pengaduan.status)}`}
                >
                ...
                <div className="bg-emerald-50 border border-emerald-100 p-4 rounded text-xs space-y-2 font-medium">
                  <div>
                    ...
                    <p className="text-gray-700 mt-0.5">
                      {pengaduan.dibahas_oleh?.name || 'Admin'}
                    </p>
                  </div>
                  <div>
                    <strong className="text-emerald-950 font-bold block">
                    ...
                    <p className="text-gray-700 mt-0.5 font-mono">
                      {pengaduan.dibahas_at
                        ? new Date(pengaduan.dibahas_at).toLocaleDateString(
                            'id-ID',
                            {
                              weekday: 'long',
                              year: 'numeric',
