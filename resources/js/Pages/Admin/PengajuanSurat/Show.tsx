                    ...
                    <div className="md:col-span-2 space-y-6">
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                            ...
                            <dl className="grid grid-cols-2 gap-4 text-xs">
                                ...
                                <div>
                                    ...
                                    <dd className="font-bold text-gray-800 mt-1 font-mono">{surat.nim}</dd>
                                </div>
                                <div>
                                    <dt className="text-gray-400 font-mono tracking-wider uppercase">Email</dt>
                                    <dd className="font-medium text-gray-800 mt-1 font-mono">{surat.email}</dd>
                                </div>
                                <div>
                                    <dt className="text-gray-400 font-mono tracking-wider uppercase">No. HP / WhatsApp</dt>
                                    <dd className="font-medium text-gray-800 mt-1 font-mono">{surat.no_hp || '-'}</dd>
                                </div>
                                <div className="col-span-2">
                                    <dt className="text-gray-400 font-mono tracking-wider uppercase">Mata Kuliah Praktikum</dt>
                                    <dd className="font-bold text-gray-800 mt-1">{surat.matkul}</dd>
                                </div>
                                <div>
                                    <dt className="text-gray-400 font-mono tracking-wider uppercase">Kelas / Sesi</dt>
                                    <dd className="font-medium text-gray-800 mt-1">{surat.kelas_sesi || '-'}</dd>
                                </div>
                                <div>
                                    <dt className="text-gray-400 font-mono tracking-wider uppercase">Tanggal Praktikum</dt>
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <span className="text-xs text-gray-400 font-mono tracking-wider uppercase block mb-1">Alasan Diajukan:</span>
                                ...
                                {surat.jenis_surat === 'Pindah Sesi' && (
                                    <div className="grid grid-cols-2 gap-4 bg-blue-50/50 border border-blue-100 p-4 rounded-lg">
                                        <div>
                                            <span className="text-xs text-gray-400 font-mono tracking-wider uppercase">Sesi Asal</span>
                                            <p className="text-sm font-bold text-[#203971] mt-0.5">{surat.sesi_asal}</p>
                                        </div>
                                        <div>
                                            <span className="text-xs text-gray-400 font-mono tracking-wider uppercase">Sesi Tujuan</span>
                                            <p className="text-sm font-bold text-[#203971] mt-0.5">{surat.sesi_tujuan}</p>
                                        </div>
                                    </div>
                                )}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                            ...
                            <div className="text-center py-4 space-y-4">
                                <span className={`inline-block px-4 py-1.5 rounded-full text-xs font-black font-mono tracking-widest border uppercase ${getStatusStyle(surat.status)}`}>
                                    {surat.status}
                                </span>
                                ...
                                {surat.status === 'Disetujui' && (
                                    <div className="bg-emerald-50 border border-emerald-100 p-4 rounded text-left text-xs space-y-3 font-medium">
                                        <div>
                                            <strong className="text-emerald-950 font-bold block mb-1">Catatan Admin:</strong>
                                            <p className="text-gray-700 italic">{surat.catatan || 'Tidak ada catatan.'}</p>
                                        </div>
                                        {suratUrl && (
                                            <div className="pt-1">
                                                <a
                                ...
                                <form onSubmit={handleApproveSubmit} className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => formApprove.setData('catatan', e.target.value)}
                                            rows={3}
                                        ...
                                        {formApprove.errors.catatan && (
                                            <span className="text-red-500 text-[10px] mt-1 block">{formApprove.errors.catatan}</span>
                                        )}
                                    </div>
                        ...
                        {action === 'reject' && (
                            <div className="bg-white border border-rose-200 rounded-lg p-5 shadow-sm space-y-4">
                                <h4 className="font-bold text-xs text-rose-800 font-mono tracking-wider uppercase">ALASAN PENOLAKAN</h4>
                                <form onSubmit={handleRejectSubmit} className="space-y-4">
                                    <div>
                                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Tulis Alasan Penolakan</label>
                                        <textarea
                                            value={formReject.data.catatan}
                                            onChange={(e) => formReject.setData('catatan', e.target.value)}
                                            required
                                            rows={4}
                                        ...
                                        {formReject.errors.catatan && (
                                            <span className="text-red-500 text-[10px] mt-1 block">{formReject.errors.catatan}</span>
                                        )}
                                    </div>
                                    <div className="flex gap-2">
                                        <button
