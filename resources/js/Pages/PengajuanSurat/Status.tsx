...
export default function Status({ pengajuan, downloadUrl }: PengajuanProps) {
  ...
  return (
    <AppLayout>
      <Head
        title={`Status Pengajuan ${pengajuan.nomor_pengajuan} - PRAKTISI`}
      />
        ...
        <div className="max-w-2xl mx-auto px-6 md:px-8">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm mb-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-gray-100">
              <div>
                ...
                <h2 className="text-2xl font-black text-[#203971] mt-0.5 tracking-tight">
                  {pengajuan.nomor_pengajuan}
                </h2>
                <p className="text-xs text-gray-500 mt-1 font-mono">
                  {new Date(pengajuan.created_at).toLocaleString('id-ID')} WIB
                </p>
              </div>
              ...
              <div className="flex flex-col items-start md:items-end gap-1.5">
                ...
                <div
                  className={`px-4 py-1.5 rounded-full text-xs font-black font-mono tracking-widest border uppercase ${getStatusColor(pengajuan.status)}`}
                >
                  {pengajuan.status}
                </div>
              </div>
            ...
            <div className="py-8">
              <div className="relative flex justify-between items-center w-full max-w-md mx-auto">
                ...
                <div className="relative z-10 flex flex-col items-center">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center shadow-sm border-2 ${
              ...
              <div className="bg-emerald-50 border border-emerald-100 p-5 rounded-xl text-center space-y-4 mb-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-emerald-600/80 mt-1">
                    Silakan unduh dokumen PDF surat resmi Anda melalui tombol di
                    bawah ini.
                  </p>
                </div>
                {pengajuan.catatan && (
            ...
            {pengajuan.status === 'Ditolak' && (
              <div className="bg-rose-50 border border-rose-100 p-5 rounded-xl space-y-3 mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-rose-800">
                      Pengajuan Ditangguhkan / Ditolak
                    </h4>
                    <p className="text-xs text-rose-600/80 mt-0.5">
                      Mohon tinjau alasan penolakan di bawah ini.
                    </p>
                  </div>
                </div>
                <div className="bg-white/60 border border-rose-200/50 rounded-lg p-3 text-xs text-rose-900 font-medium">
                  ...
                  {pengajuan.catatan ||
                    'Tidak ada alasan khusus yang dicantumkan.'}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm">
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
              <div>
                <dt className="text-xs text-gray-400 font-mono tracking-wider uppercase">
                  Nama Lengkap
                </dt>
              ...
              <div>
                ...
                <dd className="font-medium text-gray-800 mt-1 font-mono">
                  {new Date(pengajuan.tanggal_praktikum).toLocaleDateString(
                    'id-ID',
                    {
                      weekday: 'long',
                      year: 'numeric',
