...
<section class="p-8">
    ...
    <div class="mb-8 border-b border-gray-200">
        <nav class="-mb-px flex space-x-8" aria-label="Tabs">
            <a href="{{ route('jadwal.index') }}" class="border-[#203971] text-[#203971] whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm">
                Jadwal Utama
            </a>
    ...
    <form action="{{ route('jadwal.index') }}" method="GET" class="bg-white p-6 rounded-xl border border-gray-200 mb-8 flex flex-wrap gap-4 items-end shadow-sm">
        <div class="flex-grow max-w-sm">
            ...
            <div class="relative">
                ...
                <select name="ruangan_id" onchange="this.form.submit()" class="w-full bg-gray-50 border border-gray-300 rounded-lg pl-10 pr-4 py-2.5 text-gray-800 focus:ring-2 focus:ring-[#203971] focus:border-[#203971] outline-none appearance-none transition-all">
    ...
    <form action="{{ route('jadwal.simpan') }}" method="POST">
                ...
                <table class="w-full border-collapse text-left text-sm">
                    <thead>
                        <tr class="bg-[#203971] text-white">
                            <th class="p-4 border-b border-[#152a55] border-r border-[#152a55]/50 font-bold w-32 text-center uppercase tracking-wider">Hari</th>
                            <th class="p-4 border-b border-[#152a55] border-r border-[#152a55]/50 font-bold w-48 text-center uppercase tracking-wider">Jam</th>
                            <th class="p-4 border-b border-[#152a55] border-r border-[#152a55]/50 font-bold w-[35%] uppercase tracking-wider">Kelas & Angkatan</th>
                            <th class="p-4 border-b border-[#152a55] font-bold uppercase tracking-wider">Mata Kuliah</th>
                        </tr>
                    </thead>
                        ...
                        @foreach($hariJadwal as $hari => $jamList)
                            ...
                            @foreach($jamList as $index => $jam)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    @if($index == 0)
                                        <td class="p-4 font-extrabold text-[#203971] align-middle text-center bg-gray-50/50 border-r border-gray-200" rowspan="{{ $hari == 'JUMAT' ? $slotCount + 1 : $slotCount }}">
                                            {{ $hari }}
                                        </td>
                                    @endif
                                    ...
                                    <td class="p-2 border-r border-gray-200">
                                        <select name="jadwal[{{ $hari }}][{{ $jam }}][kelas_id]" class="w-full bg-transparent border border-gray-300 rounded p-2.5 focus:ring-2 focus:ring-[#203971] text-sm outline-none transition-shadow">
                                    ...
                                    <td class="p-2">
                                        <select name="jadwal[{{ $hari }}][{{ $jam }}][mata_kuliah_id]" class="w-full bg-transparent border border-gray-300 rounded p-2.5 focus:ring-2 focus:ring-[#203971] text-sm outline-none transition-shadow">
                                            <option value="">-- Kosong --</option>
        ...
        <div class="flex justify-end">
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-lg font-bold shadow-md transition-all flex items-center gap-2 tracking-wider">
